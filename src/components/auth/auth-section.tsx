...
function resolveAuthErrorMessage(rawMessage: string, isSignUp: boolean) {
  ...
  if (normalized.includes("user already registered")) {
    return "アカウントの作成に失敗しました。入力内容をご確認いただくか、ログインをお試しください。";
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap w-full gap-3">
        ...
        <button
          ...
          className={
            isMistVariant
              ? "flex flex-1 items-center justify-center gap-2 rounded-md border border-white/45 bg-white/90 px-4 py-2 text-sm text-wa-black transition-colors hover:bg-white disabled:opacity-50"
              : "flex flex-1 items-center justify-center gap-2 rounded-md border border-gray-400 bg-white px-4 py-2 text-sm disabled:opacity-50"
          }
        >
        ...
        <button
          ...
          className={
            isMistVariant
              ? "flex flex-1 items-center justify-center gap-2 rounded-md border border-white/40 bg-black/85 px-4 py-2 text-sm text-white transition-colors hover:bg-black disabled:opacity-50"
              : "flex flex-1 items-center justify-center gap-2 rounded-md bg-black px-4 py-2 text-sm text-white disabled:opacity-50"
          }
        >
      ...
      <p className={`text-center text-[11px] leading-relaxed ${isMistVariant ? "text-white/50" : "text-gray-400"}`}>
        ご利用により、
        <a href="/terms" target="_blank" rel="noopener noreferrer" className="underline underline-offset-2">利用規約</a>
        および
